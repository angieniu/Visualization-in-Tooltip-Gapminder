...
function makeScatterPlot(csvData) {
    ...
    data1980 = csvData.filter((data) => {return data.year == "1980"})
    let dropDown = d3.select("#filter").append("select")
        .attr("name", "year");

    // get arrays of fertility rate data and life Expectancy data
    let fertility_rate_data = data1980.map((row) => parseFloat(row["fertility"]));
    let life_expectancy_data = data1980.map((row) => parseFloat(row["life_expectancy"]));
    ...
    let distinctYears = [...new Set(data1980.map(d => d.year))];
...
function drawAxes2(limits, x, y, svgContainer, msm) {
    ...
    // function to scale y
    let yScale = d3.scaleLinear()
        .domain([(limits.yMax) / 1000000 + 5, (limits.yMin) / 1000000 - 5]) // give domain buffer
        .range([0 + msm.marginAll, msm.height - msm.marginAll])
